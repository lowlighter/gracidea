...
//Detailed XML maps checks
  const data = xmlparse(await Deno.readTextFile("maps/overworld/map.tmx")) as test
  {
    ...
    const npcs = [...new Set(Object.keys(JSON.parse(await Deno.readTextFile("client/static/copyrighted/textures/npcs.json")).frames).map((key:string) => key.replace(/_(?:down|up|left|right)_\d$/, "")))]
    for (const group of data.map.objectgroup) {
      ...
      for (const object of group.object) {
        ...
        Deno.test(`${layer} ${name} integrity: has valid polygon`, () => assert(((typeof width === "number")&&(typeof height === "number"))||((polygon["@points"]?.match(/-?\d+/g)?.length ?? -1)%2 === 0)))
        switch (layer) {
          case "regions":{
            regions.push(name)
...

  {
    const forbidden = [
      0, //Undefined tile
      2374, 2375, 2376, 2377, 2378, 2379, 2380, 2381, //Sea tiles
    ]
